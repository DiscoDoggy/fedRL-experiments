...
def main():
    ...
    try:
        for k in clients_per_round: 
            ...
            results_parent_path: str = f'results_for_runs_cifar_ddqn_replay_buffer'
            ...
            for epoch in range(num_rounds):
                ...
                # Push one transition per selected client, then train from replay buffer
                for client_idx in selected_client_idxs:
                    agent.push(state, client_idx, reward, next_state)
                agent.train()

                # Epsilon decay and periodic target network update (Double DQN)
                agent.round += 1
                agent.update_epsilon()
                if (epoch + 1) % agent.target_update_freq == 0:
                    agent.update_target()
                    logging.info(f"   Updated target network (round {epoch + 1}), epsilon={agent.epsilon:.4f}")
