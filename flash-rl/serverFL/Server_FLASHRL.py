...
class Server_FLASHRL(object):
    ...
    def create_clients(self, learning_rate, momentum):
        ...
        for client_name in self.dict_clients.keys():
            ...
            client = Client.Client(
                ...
                momentum,  # momentum
                self.device  # device for computations
            )
    ...
    def flatten(self, weight):
        ...
        for param in weight.values():
            ...
            if isinstance(param, torch.Tensor):
                ...
                param_numpy = np.array(param)
            weight_flatten.append(param_numpy.reshape(-1))
    ...
    def global_train(self, comms_round, C, E, mu, lamb, rep_init, batch_size, verbose_test=1, verbos=0, type_data="others", init_weights=None, init_commround=None, reputation_init=None, checkpoint_dir=None):
        '''
        ...
        '''
        if type_data == "Fall":
            return self.global_train_fall(comms_round, C, E, mu, lamb, rep_init, batch_size, verbose_test, verbos, type_data, init_weights, init_commround, reputation_init, checkpoint_dir)
        else:
            return self.global_train_others(comms_round, C, E, mu, lamb, rep_init, batch_size, verbose_test, verbos, type_data, init_weights, init_commround, reputation_init, checkpoint_dir)


    def global_train_others(self, comms_round, C, E, mu, lamb, rep_init, batch_size, verbose_test = 1, verbos = 0, type_data = "others", 
                            init_weights = None, init_commround = None, reputation_init = None, checkpoint_dir = None):
        '''
        ...
        # Track selected clients and their contributions per round
       ...
       # Initialize the first state
        ...
        for comm_round in  range(comm_start, comms_round):
            ...
            # Log selected clients for reconstruction from logs if run crashes
            if (verbos == 1):
                print(f"  Selected clients: {active_clients_index}")
            ...
            if (verbose_test == 1):
                ...
                acc_value = Accuray_global_t.cpu().numpy() if isinstance(Accuray_global_t, torch.Tensor) else Accuray_global_t
                acc_delta_value = acc_delta.cpu().numpy() if isinstance(acc_delta, torch.Tensor) else acc_delta
                print("Training round n :", (comm_round+1),", Test accuarcy : ", round(acc_value*100, 2) ,", Test loss :", round(loss_test, 2), ", Acc delta:", round(acc_delta_value*100, 4))
                print("*************************************************************************************")
            ...
            # Record utility scores for selected clients in this round
            utility_scores_per_round.append(utility_clients.copy())
            print(f"  Utility scores: {dict(zip(active_clients_index, utility_clients))}")
            ...
            print(f"  Rewards: {dict(zip(active_clients_index, reward))}")
            ...
            # Save checkpoint every 10 rounds (overwrites previous checkpoint)
            if checkpoint_dir is not None and (comm_round + 1) % 10 == 0:
                import os
                os.makedirs(checkpoint_dir, exist_ok=True)
                checkpoint = {
                    'comm_round': comm_round + 1,
                    'global_model_weights': self.model.state_dict(),
                    'dql_main_network': dql.main_network.state_dict(),
                    'dql_target_network': dql.target_network.state_dict(),
                    'dql_epsilon': dql.epsilon,
                    'reputation': reputation_clients_t,
                    'state': state,
                    'best_model_weights': best_model_weights,
                    'best_accuracy': best_accuracy,
                    'accuracy_history': accuarcy,
                    'loss_history': loss,
                    'time_rounds': time_rounds,
                    'time_rounds_sum': time_rounds_sum,
                    'rewards': rewards,
                    'loss_dql_history': list_loss_DQL,
                    'selected_clients_per_round': selected_clients_per_round,
                    'accuracy_deltas': accuracy_deltas,
                    'utility_scores_per_round': utility_scores_per_round,
                    'reputation_list': reputation_list
                }
                torch.save(checkpoint, os.path.join(checkpoint_dir, 'checkpoint_latest.pt'))
                print(f"  [Checkpoint saved at round {comm_round + 1}]")
