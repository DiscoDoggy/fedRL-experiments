...
from models.CIFAR10.ResNet import ResNetFed
...
CLIENTS_PER_ROUND_LIST = [5,10]  # List of clients per round to run experiments for
...
# ==============================================================================
# Creation of a bias-based non-IID division of CIFAR-10
# ==============================================================================
...
# Define transformations for CIFAR-10 with empirical channel-wise statistics
# Training transform with data augmentation
trans_cifar_train = transforms.Compose([
    transforms.RandomCrop(32, padding=4),  # Random crop with padding for augmentation
    transforms.RandomHorizontalFlip(),  # Random horizontal flip
    transforms.ToTensor(),  # Convert images to PyTorch tensors
    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))  # CIFAR-10 empirical mean and std
])
...
dataset_test = datasets.CIFAR10('data/cifar10/', train=False, download=True, transform=trans_cifar_test)
...
client_dict_bias, client_preferences = bias_partitioner.bias_partition_cifar10(
...
for client_id in range(NUM_CLIENTS):
    ...
    client_labels = [dataset_train.targets[idx] for idx in indices]
...
for CLIENTS_PER_ROUND in CLIENTS_PER_ROUND_LIST:
    ...
    # Initialize ResNetFed model (modified ResNet-18 from FedRL)
    model_CIFAR10 = ResNetFed(num_classes=10)

    # Move model to GPU if available
    model_CIFAR10 = model_CIFAR10.to(device)
    print(f"ResNetFed model initialized and moved to {device}")
    ...
    Serveur_FLASHRL = Server_FLASHRL.Server_FLASHRL(
        ...
        B=32,  # Local batch size for training on each client
        dataset_test=dataset_test,  # Testing dataset for evaluation
        learning_rate=0.001,  # Learning rate (note: Client now uses Adam with lr=0.001)
    ...
    results_dir = f'flash_rl_results_lower_bs/{EXPERIMENT_ID}'
    ...
    results = Serveur_FLASHRL.global_train(
        ...
        batch_size=32,  # Batch size used in each local training iteration
