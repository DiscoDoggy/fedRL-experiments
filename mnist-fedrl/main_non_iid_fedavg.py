...
from models import SimpleNN, SimpleMNISTCNN
from non_iid_distributor import NonIIDDataDistributor, NonIIDAnalyzer, create_non_iid_config
from direchlet_partition import direchlet_partition, plot_stacked_client_class_distributions
...
def fedavg_select_random_clients(num_clients: int, k: int):
...
def main():
    ...
    clients_per_round: list[int] = [5, 10, 20, 30, 40, 50, 60]
    # clients_per_round: list[int] = [2,3]

    
    try:
        for k in clients_per_round:
            num_clients = 100
            #k = 5  # clients per round
            num_classes = 10
            num_rounds = 200
            ...
            results_parent_path: str = f'results_for_runs_mnist_fedavg'
            per_run_path: str = f'{num_clients}_clients_{k}_per_round_mnist'
            ...
            cifar_fedavg_runs_pth = 'mnist_fedavg_runs'
            cifar_fedavg_runs_checkpoint = f'{cifar_fedavg_runs_pth}/{k}_clients'
            os.makedirs(cifar_fedavg_runs_checkpoint, exist_ok=True)
            ...
            logging.info(f"Loaded MNIST dataset: {len(mnist_dataset)} train, {len(mnist_test_dataset)} test")
            ...
            for i in range(num_clients):
                ...
                # Use SimpleNN for MNIST
                client.model = SimpleMNISTCNN().to(client.device)
            ...
            logging.info(f" Created {num_clients} clients with SimpleNN models")
            
            # Create server with SimpleNN
            server = Server(mnist_test_dataset, num_classes)
            server.global_model = SimpleMNISTCNN().to(server.device)
            
            logging.info("Initialized server with SimpleNN model")
            ...
            for epoch in range(num_rounds):
                ...
                selected_client_idxs = fedavg_select_random_clients(num_clients, k)
            ...
            plt.title(f'Global Model Accuracy Over Rounds on MNIST with {num_clients} Clients')
            plt.xlabel('Round')
            plt.ylabel('Accuracy')
            ...
            plt.title(f'Global Model Loss Over Rounds on MNIST with {num_clients} Clients')
            plt.xlabel('Round')
            plt.ylabel('Loss')
            plt.grid(True, alpha=0.3)
            ...
            plt.ylim(0, 1)
            plt.tight_layout()
            # plt.savefig('individual_plots/global_accuracy.png', dpi=300, bbox_inches='tight')
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            # plt.savefig('individual_plots/global_loss.png', dpi=300, bbox_inches='tight')
            ...
            # plt.grid(True, alpha=0.3)
            # plt.tight_layout()
            # # plt.savefig('individual_plots/rewards.png', dpi=300, bbox_inches='tight')
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            # plt.savefig('individual_plots/client_participation.png', dpi=300, bbox_inches='tight')
            ...
            # plt.grid(True, alpha=0.3)
            # plt.tight_layout()
            # # plt.savefig('individual_plots/data_distribution_stats.png', dpi=300, bbox_inches='tight')
